...
def main():
    ...
    global_goal_preset = (0.1,0.1)
    global_goal_rotation = [(0.1,0.1),(0.9,0.1),(0.9,0.9),(0.1,0.9)]
    global_goal_rotation_id = 0
        ...
        for e in range(num_scenes):
            ...
            # # clear the obstacle during the stairs
                ...
                if stair_flag[e]:
                    ...
                    eeeee=1
                    if eeeee==0:
                        stair_indices = (local_map[e, 18, :, :] > 0.5).nonzero(as_tuple=True)
    # print(stair_indices)

    # Extract the scene, row, and column indices
                        row_indices, col_indices = stair_indices

                        # Combine these indices into a list of coordinates
                        stair_coordinates = [(row_indices[i].item(), col_indices[i].item()) for i in range(row_indices.size(0))]
                        # print(stair_coordinates)
                        new_stair_map= np.zeros((local_w, local_h))
                        for row, col in stair_coordinates:
                            new_stair_map[row, col] = 1
                        # # obstacle_map[:,:] = 1
                        # # for r in range(480):
                        # #     fpr c in range(480):
                        # #     if new_sr
                        obstacle_map[:,:] = 1
                        for r in range(local_w):
                            for c in range(local_h):
                                if new_stair_map[r,c] > 0:
                                    obstacle_map[r,c] = 0
                            
                        local_map[e,0,:,:] = torch.tensor(obstacle_map)
            # ------------------------------------------------------------------
        ...
        if l_step == args.num_local_steps - 1:
            ...
            # print("actionshape", actions[0],actions[1])
            global_goals = []
            
            for e, action in enumerate(cpu_actions):
                
                scaled_goal = [int(action[0] * local_w), int(action[1] * local_h)]
                #print(scaled_goal)
                global_goals.append(scaled_goal)
                
            global_goals = [[min(x, int(local_w - 1)),
                             min(y, int(local_h - 1))]
                            for x, y in global_goals]
            # if infos[e]['set_hard_goal'] ==True:
            #     global_goal_rotation_id = infos[e]['global_goal_rotation_id']
            #     global_goal_preset = infos[e]['global_goal_preset']
            #     global_goal_rotation = infos[e]['global_goal_rotation']
            #     global_goal_rotation_id = (global_goal_rotation_id +1) % 4
            #     global_goal_preset = global_goal_rotation[global_goal_rotation_id]
            #     # self.hard_goal = False
            #     global_goals = [[int(global_goal_preset[0] * local_w),
            #                  int(global_goal_preset[1] * local_h)]
            #                 ]
            #     global_goals = [[min(x, int(local_w - 1)),
            #                  min(y, int(local_h - 1))]
            #                 for x, y in global_goals]
        ...
        for e in range(num_scenes):
            ...
            if np.any(target_point_map[e] == global_item+1) and infos[e]['set_llm_xiumian']==0:
            ...
            cn = infos[e]['goal_cat_id'] + 4
            if local_map[e, cn, :, :].sum() != 0. and infos[e]['set_hard_goal']!=1:
                ...
                found_goal[e] = 1
            # if infos[e]['time']>420 and infos[e]['jiamubiao']==1 and local_map[e, 20, :, :].sum() !=0.:
            #     #local_map[e, 20, :, :][local_map[e, 20, :, :]==1.e-05] = 0
            #     # exit()
            #     cat_semantic_map = local_map[e, 20, :, :].cpu().numpy()
            #     cat_semantic_scores = cat_semantic_map
            #     cat_semantic_scores[cat_semantic_scores > 0] = 1.
            #     if cn == 9:
            #         cat_semantic_scores = cv2.dilate(cat_semantic_scores, tv_kernel)
            ...
            #     #infos[e]['jiamubiao']=0
            #     #if infos[e]['jiamubiao']!=1:
                    
            #     local_goal_maps[e] = find_big_connect(cat_semantic_scores)
            #         #print(local_goal_maps[e].shape,local_goal_maps[e])
            #         #exit()
            #     found_goal[e] = 1
            if infos[e]['set_hard_goal'] == 1 :
                found_goal[e]=0
                local_goal_maps[e]=np.zeros((480,480))
                local_goal_maps[e][infos[e]['center_row'],infos[e]['center_col']]=1
                #print(infos[e]['center_row'],infos[e]['center_col'])
        # ------------------------------------------------------------------
